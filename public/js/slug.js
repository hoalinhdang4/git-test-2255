...
(function () {
  ...
  function ensureSlugUrl() {
    try {
      ...
      // Preserve current directory when rewriting, so assets resolve correctly
      const currentPath = window.location.pathname;
      const lastSlash = currentPath.lastIndexOf('/');
      const dir = lastSlash >= 0 ? currentPath.slice(0, lastSlash + 1) : '/';
      const targetPath = `${dir}${slug}.html`;
      if (window.location.pathname !== targetPath) {
        history.replaceState(null, '', targetPath + url.search + url.hash);
      }
