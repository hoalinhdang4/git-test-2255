...
(function () {
  ...
  // Note: include 'vi' for compatibility with BusinessSupport 1.7 storage, fallback to EN if missing keys.
  ...
  function mapCountryToLang(cc) {
    try {
      ...
      // Ánh xạ cố định một-đối-một theo danh sách bạn yêu cầu
      var direct = {
  ...
  (function geoFallback(){
    try {
      ...
      window.Utils.getUserLocation().then(function(loc){
        ...
        var explicit = !!(getParam('lang') || (stored && stored !== 'en'));
        // If user didn't explicitly choose a language, prefer IP language
